...
// printJSONReportMulti outputs lint reports for multiple files as JSON.
func printJSONReportMulti(issues []lint.Issue) {
	type IssueOut struct {
		Rule     string `json:"rule"`
		Severity string `json:"severity"`
		Message  string `json:"message"`
	...
	out := struct {
		...
		Issues: make([]IssueOut, 0, len(issues)),
		Score:  lint.CalculateScore(issues),
		Grade:  lint.ScoreToGrade(lint.CalculateScore(issues)),
	}
...
// printGitHubReportMulti outputs issues in GitHub Actions annotation format for multiple files.
func printGitHubReportMulti(issues []lint.Issue) {
	for _, issue := range issues {
		switch issue.Severity {
		case lint.SeverityCritical, lint.SeverityError:
			fmt.Fprintf(os.Stderr, "::error file=%s,line=%d,col=%d::[%s] %s\n",
